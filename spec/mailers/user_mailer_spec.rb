...
describe UserMailer do
  ...
  context "#notify_build_success" do
    before do
      ...
      @build = FactoryGirl.create(:build, repository: @repository, last_commit_id: "123456789", branch: "develop", last_commit_message: "hello", duration: 20)
    end

    subject { UserMailer.notify_build_success(@build) }
    it { should deliver_to("user@gmail.com") }
    it { should have_subject("[rails-brakeman] flyerhzm/test build #1") }
